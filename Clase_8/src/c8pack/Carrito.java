...
/*******************************************************************************/
	public void addProd2Carrito(Producto prod, int cant) throws NoHayStockException {
		...
		if	(prod.setCantProd(cant)) {
			...
			System.out.println("ADD-> ("+String.valueOf(cant)+") _"+prod.getCodigo()+"_ PU$ "+String.valueOf(prod.getPrecio()));
		}
		else throw new NoHayStockException(prod);
	}
	
/*******************************************************************************/
	public float costoFinal(Descuento des) throws DescuentoPrecioCeroException {
		float acum=0;
		...
		if (acum>0) {
			try {
				...
				acum = 0;
				System.out.println("No se puede aplicar descuento: costo negativo!");
				};
		}
		else throw new DescuentoPrecioCeroException(this); 
